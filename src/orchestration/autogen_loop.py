...
def run_autogen_brainstorm(
    idea: str, participants: List[Dict[str, str]], provider: str = "openai", model: str = None
) -> Dict[str, Any]:
    try:
        ...
        for p in participants:
            role = autogen.AssistantAgent(
                ...
                system_message=f"You are {p.get('role', 'member')}. {system_message}",
            )
            agents.append(role)
        # Moderator
        moderator = autogen.UserProxyAgent(name="Moderator", human_input_mode="NEVER")
        # Start chat
        chat_result = autogen.GroupChat(
        ...
        manager.initiate_chat(
            agents[0], message="Let's break down the idea and propose 3 next steps."
        )
        transcript = [m for m in chat_result.messages]
        summary_prompt = (
            "Summarize the group chat into decisions, risks, and 5 concrete next steps."
        )
...
Return JSON with keys: decisions (list), risks (list), next_steps (list of 5)."""
        ...
        return {
            "transcript": [{"role": "system", "content": "fallback"}],
            "summary": summary,
            "engine": "fallback",
        }
