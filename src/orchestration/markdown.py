...
def render_brainstorm(d: dict) -> str:
    lines = [
        f"# Brainstorm — {d.get('topic', '')}",
        "",
        f"**Facilitator:** {d.get('owner', '')}",
        "",
        "## Idea Map",
    ]
    ...
    for i, c in enumerate(d.get("top3", []), start=1):
        lines.append(f"{i}. {c}")
    lines += ["", "## Decision Memo"]
    dec = d.get("decision", {})
    if dec:
        lines += [
            f"- Problem: {dec.get('problem', '')}",
            f"- Option chosen: {dec.get('option', '')}",
            f"- Why now: {dec.get('why', '')}",
        ]
    return "\n".join(lines)
